...
function getClientId(params: URLSearchParams, iss: string): string {
  ...
  // Default to iEHR client ID
  return IEHR_CLIENT_ID;
}
...
function setupIEHRClient(tokenData: TokenResponse, iss: string, iehrContext: { iehr: IEHRClient }): void {
  // Store the access token and other relevant data
  sessionStorage.setItem('smart_patient', tokenData.patient);

  // Configure the iEHR client
  iehrContext.iehr = new IEHRClient({
    baseUrl: iss,
    fhirUrlPath: '',
...
export function LaunchPage(): JSX.Element {
  ...
  const iehrContext = useIEHRContext();

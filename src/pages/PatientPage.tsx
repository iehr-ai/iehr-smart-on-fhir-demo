...
import { calculateAgeString, formatDate, formatDateTime, formatHumanName } from '@iehr/core';
import { HumanName, Observation, Patient } from '@iehr/fhirtypes';
import { ResourceAvatar, useIEHR } from '@iehr/react';
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      try {
        ...
        if (!iehr.getAccessToken() || !patientId) {
          throw new Error('No authentication token or patient ID found');
        }

        const patient = await iehr.readResource('Patient', patientId);
        setPatient(patient);

        // Fetch blood pressure readings
        const bpObservations = await iehr.searchResources('Observation', {
